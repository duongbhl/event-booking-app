...
exports.generateEventContentSuggestion = void 0;
...
const buildPrompt = ({ category, location, date, existingTitle, existingDescription, userPrompt, language = "en", }) => {
...
const sanitizeSuggestion = (parsed) => {
...
const titleCase = (value) => value
    ...
    .join(" ");
const buildTemplateSuggestion = (input) => {
    ...
    if (input.language === "vi") {
        return sanitizeSuggestion({
            title: input.existingTitle?.trim() ||
                `${categoryLabel} Experience tại ${city}`,
            description: input.existingDescription?.trim() ||
                [
...
const generateWithOpenAI = async (input) => {
    ...
    return sanitizeSuggestion(JSON.parse(rawText));
};
const generateWithOllama = async (input) => {
    const response = await axios_1.default.post(process.env.OLLAMA_API_URL || DEFAULT_OLLAMA_URL, {
...
const generateEventContentSuggestion = async (input) => {
    ...
    if (preferredProvider === "ollama") {
        try {
            ...
            console.error("Ollama suggestion failed:", ollamaError?.response?.data || ollamaError?.message || ollamaError);
    ...
    try {
        ...
        console.error("OpenAI suggestion failed:", openAiError?.response?.data || openAiError?.message || openAiError);
...
exports.generateEventContentSuggestion = generateEventContentSuggestion;
